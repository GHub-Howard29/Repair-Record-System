...
- Phase 6 匯出已補上：單筆 PDF 在電腦維持瀏覽器列印另存；手機／平板以逐頁 Canvas 產生 A4 實際 PDF 後開啟系統 App 選擇面板，交由 PDF 閱讀器、檔案管理或雲端 App 預覽與儲存，可避免長內容產生空白 PDF。檔名為 `維修報告_YYYYMMDD_客戶名稱.pdf`；手機匯出時會保留完整的置中公司名稱，且附件清單會另起一張 A4 頁。無附件時略過附件清單頁；全部資料可匯出含「維修紀錄／收費明細」兩張工作表的 `.xlsx`。
