...
- 登出與關閉 APP 前會提示先儲存；桌機右上提供登出與關閉 APP，手機將關閉 APP 放於頁首、帳號與登出放於功能選單底部。
- 維修紀錄核心規則已落地：必填欄位、完成後鎖定、同製造號碼不可重複建立未完成案件、保固提示。
- Phase 2 部分能力已補上：故障零件自動產生零件收費欄位、同製造號碼歷史維修摘要。
- Phase 3 附件管理已補上：最多六張圖片、超過 1.5 MB 自動壓縮、預覽、更換、刪除、完成後鎖定；附件異動會先寫入本機並立即恢復操作，背景佇列再依序同步 Firestore 與 Google Drive。已上傳照片更換時會上傳新檔並刪除舊檔；上傳途中刪除或換版時會清理稍後完成的過期雲端檔案，附件同步狀態以中文顯示。
- Phase 5 查詢已補上：姓名、製造號碼、回送地點、收到日期區間與更換零件即時篩選；零件選項沿用故障零件清單，可快速搜尋歷史更換紀錄。輸入查詢條件時顯示全部狀態，統計卡可明確切換維修中／已完成。
- Phase 6 匯出已補上：單筆 PDF 在電腦維持瀏覽器列印另存；手機／平板以逐頁 2 倍解析度 Canvas 產生高品質 A4 實際 PDF 後開啟系統 App 選擇面板，交由 PDF 閱讀器、檔案管理或雲端 App 預覽與儲存，可避免長內容產生空白 PDF。檢修中表頭與檔名使用「維修檢測與費用說明」，完成後使用「維修完工報告」；PDF 標題以原始 NICE GREEN 彩色 LOGO 與置中公司名稱組合呈現，附件清單會另起一張 A4 頁，並依維修前、維修中、維修後、其他的固定順序輸出，照片會保留原始長寬比例，且照片與說明不會跨頁切割。無附件時略過附件清單頁；全部資料可匯出含「維修紀錄／收費明細」兩張工作表的 `.xlsx`。
- 已建立 Vitest 自動化測試基礎，涵蓋製造號碼、完成判定、Excel 匯出資料、列印附件頁判定，以及附件連續新增與上傳途中刪除等同步競態。
- 可替換式服務架構已補上：維修資料、附件儲存、匯出、同步佇列與同步 processor 皆有獨立介面或模組。
- Firebase / Firestore 維修文字資料同步已串接，Google 登入、雲端寫入與跨電腦讀取已完成實機驗證。
- Drive 附件代理與草稿照片、照片說明、拍照入口、預覽、故障選項及同步提示已實作；`uploadRepairAttachment` 與 `deleteRepairAttachment` 均已部署。重新載入時會保留本機預覽，並以 Drive 縮圖作為既有附件備援；照片新增、更換與刪除已完成三台設備即時同步驗證。
- 製造號碼會自動轉為大寫，格式固定為 `NIS-` 加 12 碼英數字（共 16 碼）。送回日期儲存前會檢核結案資料完整性，通過後才鎖定案件並切換到已完成清單。
