...
- Phase 3 附件管理已補上：最多五張圖片、超過 1.5 MB 自動壓縮、預覽、更換、刪除、完成後鎖定；附件異動會先寫入本機並立即恢復操作，背景佇列再依序同步 Firestore 與 Google Drive。已上傳照片更換時會上傳新檔並刪除舊檔；上傳途中刪除或換版時會清理稍後完成的過期雲端檔案，附件同步狀態以中文顯示。
- Phase 5 查詢已補上：姓名、製造號碼、回送地點、收到日期區間與更換零件即時篩選；零件選項沿用故障零件清單，可快速搜尋歷史更換紀錄。輸入查詢條件時顯示全部狀態，統計卡可明確切換維修中／已完成。
- Phase 6 匯出已補上：單筆 PDF 在電腦維持瀏覽器列印另存；手機／平板以逐頁 2 倍解析度 Canvas 產生高品質 A4 實際 PDF 後開啟系統 App 選擇面板，交由 PDF 閱讀器、檔案管理或雲端 App 預覽與儲存，可避免長內容產生空白 PDF。檢修中表頭與檔名使用「維修檢測與費用說明」，完成後使用「維修完工報告」；PDF 標題以原始 NICE GREEN 彩色 LOGO 與置中公司名稱組合呈現，附件清單會另起一張 A4 頁，照片會保留原始長寬比例，且照片與說明不會跨頁切割。無附件時略過附件清單頁；全部資料可匯出含「維修紀錄／收費明細」兩張工作表的 `.xlsx`。
