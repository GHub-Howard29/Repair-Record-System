...
- Phase 1 至 Phase 6 的主要使用流程已完成；Phase 7 進入實機驗收與自動化測試補強階段。
- Phase 4 的 Firestore 文字資料同步、離線暫存與失敗重試已完成，並已進行授權／未授權帳號與跨裝置人工驗證。
- Phase 4.5 的 Firebase Functions Drive 上傳代理已部署；相片已可上傳至 Drive，但已同步照片在重新載入後的預覽仍需實機複驗。
- 搜尋採收到日期起訖區間、故障分類與關鍵字；查詢條件預設跨狀態顯示，統計卡可手動指定維修中或已完成。
- 製造號碼規格為 `NIS-` 加 12 碼英數字，共 16 碼，輸入時自動轉為大寫。
- 響應式版面：寬版維持右側收費摘要與歷史維修；窄版收費摘要跟隨附件區塊，歷史維修位於維修紀錄列表底部。
